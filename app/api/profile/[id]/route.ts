...
export async function GET(_request: Request, { params }: Params) {
  ...
  const { data: listings } = await supabase.from('listings').select('*, listing_media(*)').eq('owner_id', id).eq('status', 'active').order('featured_until', { ascending: false, nullsFirst: false }).order('created_at', { ascending: false })
