...
export async function PATCH(request: Request, { params }: Params) {
  ...
  const { data, error } = await auth.supabase.from('contact_channels').update(payload).eq('id', id).select().single()
  if (error) return schemaOrDb(error, 'Unable to update channel.', 400)
  await writeAudit(auth.supabase, {
    actorId: auth.user.id,
    action: 'contact.channel.update',
...
export async function DELETE(_request: Request, { params }: Params) {
  ...
  const auth = await requireAdmin()
  if (auth.response) return auth.response
  const { error } = await auth.supabase.from('contact_channels').delete().eq('id', id)
  if (error) return schemaOrDb(error, 'Unable to delete channel.', 400)
  await writeAudit(auth.supabase, {
    actorId: auth.user.id,
    action: 'contact.channel.delete',
    entityType: 'contact_channel',
    entityId: id,
  })
