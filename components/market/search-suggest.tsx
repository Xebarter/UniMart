...
import { BriefcaseBusiness, ChevronRight, Home, Package, Search, Tag, X } from 'lucide-react'
import { ListingPhoto } from '@/components/listing-photo'
import { useMarket } from '@/components/market/provider'
import { api } from '@/lib/api-client'
import { formatUGX, rentPeriodSuffix } from '@/lib/format'
import { marketPaths } from '@/lib/market-paths'
import { groupListingsByCategory, matchingCategories, rankListings } from '@/lib/search'
...
function ResultRow({
  ...
  return (
    <button
      ...
      onClick={onChoose}
      className={`flex w-full min-w-0 items-center gap-3 rounded-xl px-1.5 py-1.5 text-left transition ${
        selected ? 'bg-[#eef5f2]' : 'hover:bg-[#f6f9f7]'
      }`}
    >
      <ListingPhoto listing={item} alt="" className="size-12 shrink-0 rounded-xl" />
...
export function SearchField() {
  ...
  const { query, setQuery, listings, setCategory } = useMarket()
  ...
  const hits = useMemo<Hit[]>(() => {
    ...
    if (query.trim() && ranked.length) next.push({ kind: 'all' })
    return next
  }, [categories, listingHits, query, ranked.length])
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      api.listings(`q=${encodeURIComponent(q)}&limit=24`)
        .then((result) => {
          if (!controller.signal.aborted) setRemote(result.data)
        })
        .catch(() => {
  ...
  return (
    <div ref={rootRef} className={`relative min-w-0 flex-1 sm:max-w-[460px] lg:max-w-[540px] ${open ? 'z-40' : ''}`}>
      ...
      <div className={`z-40 ${open ? 'absolute inset-x-0 top-0 rounded-2xl border border-[#d7e4df] bg-white shadow-[0_28px_70px_rgba(36,62,57,0.16)]' : 'relative'}`}>
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search listings and categories"
            className={`h-10 w-full min-w-0 bg-white pl-9 pr-10 text-sm text-[#243e39] outline-none placeholder:text-[#a8b2ae] sm:pl-10 ${
              open
        ...
        {open && (
          <div id={listboxId} role="listbox" aria-label="Search suggestions" className="search-tile max-h-[min(calc(100dvh-9rem),560px)] overflow-y-auto overscroll-contain border-t border-[#eef3f0] px-2.5 pb-2.5 pt-2 sm:px-3">
            <p className="px-1.5 pb-2 text-[10px] font-bold uppercase tracking-[0.16em] text-[#b08a72]">
              {empty ? 'Browse categories' : searching ? 'Finding matches' : ranked.length ? 'Related results' : 'No matches yet'}
            </p>
              ...
              {categories.map((id) => {
                ...
                return (
                  <button
                    ...
                    className={`flex min-w-0 items-center gap-2.5 rounded-xl border px-2.5 py-2 text-left transition ${
                      selected
                        ? 'border-[#8bb4a7] bg-[#eaf3ef] shadow-[0_6px_16px_rgba(49,94,85,0.08)]'
            ...
            {typed && ranked.length > 0 && (
              ...
              >
                <span>See all {ranked.length} result{ranked.length === 1 ? '' : 's'}</span>
